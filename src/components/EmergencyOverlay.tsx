...
import { ShieldAlert, Volume2, VolumeX, AlertTriangle, X } from 'lucide-react';
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="w-full max-w-md space-y-5 z-10">
          ...
          {/* Large Cancel Trigger Button (48px Touch target compatible) */}
          <button
            id="emergency-cancel-trigger"
            onClick={cancelSOS}
            className="w-full py-4 bg-slate-900 hover:bg-slate-800 border border-slate-700/80 hover:border-slate-600/80 text-white font-bold uppercase tracking-wider text-sm rounded-xl flex items-center justify-center gap-2.5 cursor-pointer shadow-lg transition-all active:scale-[0.98]"
          >
            <X className="w-4 h-4 text-red-400" />
            <span>Abort SOS Sequence</span>
          </button>
