...
export const AIHub: React.FC = () => {
  ...
  const handleChatSubmit = (e: React.FormEvent) => {
    ...
    fetch('/api/gemini/chat', {
      ...
      body: JSON.stringify({ message: userMsg, location: userLocation })
    })
      .then(res => res.json())
      .then(data => {
        const reply = data.reply || 'System encountered an error.';
      ...
      .catch(err => {
        ...
        setChatMessages(prev => [...prev, { sender: 'bot', text: 'Connection to AI framework failed.', timestamp: Date.now() }]);
        setIsTyping(false);
      });
  };
