...
export const ContactsPage: React.FC = () => {
  ...
  const updateContactToggle = async (id: string, field: keyof EmergencyContact, value: boolean) => {
    try {
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 md:p-6 shadow-2xl relative overflow-hidden group mb-8">
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {contacts.map(c => (
            <div key={c.id} className="flex flex-col gap-2 bg-slate-950 p-4 rounded-lg border border-slate-800">
              ...
              <div className="flex flex-wrap gap-4 text-sm mt-2 text-slate-400 border-t border-slate-800 pt-2">
                <label className="flex items-center gap-1 cursor-pointer">
                  <input type="checkbox" checked={c.enable_push} onChange={e => updateContactToggle(c.id, 'enable_push', e.target.checked)} />
                  Push
                </label>
                <label className="flex items-center gap-1 cursor-pointer">
                  <input type="checkbox" checked={c.enable_sms} onChange={e => updateContactToggle(c.id, 'enable_sms', e.target.checked)} />
                  SMS
                </label>
                <label className="flex items-center gap-1 cursor-pointer">
                  <input type="checkbox" checked={c.enable_whatsapp} onChange={e => updateContactToggle(c.id, 'enable_whatsapp', e.target.checked)} />
                  WhatsApp
                </label>
              </div>
            </div>
