...
class SafetyViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun login(username: String, orgId: String): Boolean {
        ...
        if (trimmedUser.equals("SafetyLink", ignoreCase = true) && trimmedOrg.equals("SL-ADMIN-000", ignoreCase = true)) {
            _currentUser.value = "Super Administrator"
            ...
            prefs.edit().apply {
                ...
                putString("current_user", "Super Administrator")
        ...
        if (trimmedUser.isBlank() || trimmedOrg.isBlank()) return false

        _currentUser.value = trimmedUser
        _currentOrgId.value = trimmedOrg
        _isLoggedIn.value = true
        setRole(UserRole.CITIZEN)

        prefs.edit().apply {
            putBoolean("is_logged_in", true)
            putString("current_user", trimmedUser)
            putString("current_org_id", trimmedOrg)
            apply()
        }
        addTelemetry("AUTH", "User $trimmedUser logged into workspace $trimmedOrg", "INFO")
        return true
    }
