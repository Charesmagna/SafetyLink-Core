...
const startMarker = '<h4 className="text-[10px] font-bold text-slate-400 uppercase border-b border-slate-800 pb-2">Personal Integrations</h4>';
const firstIndex = content.indexOf(startMarker);
const lastIndex = content.lastIndexOf('</button></div>'); // The end of OwnCloud

if (firstIndex !== -1 && lastIndex !== -1) {
  const nextDivEnd = content.indexOf('</div>', lastIndex + '</button></div>'.length);
  const endOfBlock = content.indexOf('</div>', nextDivEnd + 6); // To catch the closing tags of the fields
...
<h4 className="text-[10px] font-bold text-slate-400 uppercase border-b border-slate-800 pb-2">Personal Integrations</h4>
              ...
              <div className="space-y-3 border-t border-slate-800/50 pt-3">
                ...
                <input type="text" value={ocUrl} onChange={e => setOcUrl(e.target.value)} placeholder="Server URL (e.g. https://cloud.example.com)" className="w-full bg-slate-950 border border-slate-900 rounded-xl px-3 py-2 text-[10px] text-slate-200 focus:outline-none focus:border-purple-500/50" />
                ...
                <input type="password" value={ocToken} onChange={e => setOcToken(e.target.value)} placeholder="App Password / Token" className="w-full bg-slate-950 border border-slate-900 rounded-xl px-3 py-2 text-[10px] text-slate-200 focus:outline-none focus:border-purple-500/50" />
                <input type="text" value={ocFolder} onChange={e => setOcFolder(e.target.value)} placeholder="Target Folder (e.g. /SafetyLink)" className="w-full bg-slate-950 border border-slate-900 rounded-xl px-3 py-2 text-[10px] text-slate-200 focus:outline-none focus:border-purple-500/50" />
              </div>
`;

  const pre = content.substring(0, firstIndex);
  
  // Find where the last block really ends.
  // We look for the last </button></div>, then the end of the 4 inputs for OwnCloud, then the closing div.
  const endMarker = 'placeholder="Target Folder';
  const lastFolderInput = content.lastIndexOf(endMarker);
  let finalEnd = content.indexOf('</div>', lastFolderInput);
  finalEnd = content.indexOf('</div>', finalEnd + 6); // closing div for OwnCloud
  
  const post = content.substring(finalEnd + 6);
  content = pre + cleanBlock + post;
}

// Add state variables and connectService function if missing
const statesToAdd = `
...
if (!content.includes('const [turnApiToken')) {
  content = content.replace("const [twilioAccountSid", statesToAdd + "\n  const [twilioAccountSid");
}
