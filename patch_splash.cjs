const fs = require('fs');
const content = fs.readFileSync('src/components/SplashReveal.tsx', 'utf8');

const replacement = `import React, { useEffect, useState, useRef } from 'react';
...
export const SplashReveal: React.FC<SplashRevealProps> = ({ onComplete }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-4xl aspect-video z-10 shadow-2xl">
        <video
          autoPlay
          muted
          playsInline
...
fs.writeFileSync('src/components/SplashReveal.tsx', replacement);
