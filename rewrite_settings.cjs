...
// The file has multiple `<div className="space-y-3 border-t border-slate-800/50 pt-3">` sections after Personal Integrations.
// Let's find the first `Personal Integrations` and everything before it.
const firstInt = content.indexOf('<h4 className="text-[10px] font-bold text-slate-400 uppercase border-b border-slate-800 pb-2">Personal Integrations</h4>');
const pre = content.substring(0, firstInt);

// Find the last OwnCloud block's ending.
const endOwnCloudBtn = content.lastIndexOf('Download ZIP</button></div>');
let endDiv = content.indexOf('</div>', endOwnCloudBtn);
endDiv = content.indexOf('</div>', endDiv + 6); // closing div for inputs
endDiv = content.indexOf('</div>', endDiv + 6); // maybe another one

const post = content.substring(endDiv + 6);
...
let newContent = pre + cleanBlock + post;

// Now add the missing state and functions
const statesToAdd = `
