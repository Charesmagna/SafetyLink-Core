...
// Use a more robust match for the function
let hardwareStart = modalCode.indexOf('const handlePaystackHardwareCheckout = ');
let hardwareEnd = modalCode.indexOf('  const handlePayfastCheckout =');
let hardwareCheckoutStr = modalCode.substring(hardwareStart, hardwareEnd);

let contentStart = modalCode.indexOf('<div className="text-center mb-10">');
let contentEnd = modalCode.lastIndexOf('</motion.div>');
let contentStr = modalCode.substring(contentStart, contentEnd);
// Remove the last two </div> divs which close motion.div and the outer container. Wait, contentEnd is exactly before </motion.div>.
// Let's strip the last few closing divs of the modal if they are captured
contentStr = contentStr.replace(/<\/div>\s*<\/div>\s*$/, '</div>'); // Try to match the internal structure
